...
git clone https://github.com/krcourville/better-application-toolkit.git
...
```bash
# Nuclear clean (root "nuke" script: removes root node_modules, lockfile, then cleans all packages)
vp run nuke
vp install
...
| `vp run nuke`                 | Remove root `node_modules` and lockfile, then clean all packages                                                         |
